...
def extract_option_dict(content):
    pattern = re.compile(r"\['(.*?)'\]")
    
    match = pattern.search(content)
    
    if match:
        options_list = match.group(1).split("', '")
...
def eval_agieval_gaokao_mathqa(item, pred_key='prediction', prec=1e-3):
    pred_str = item[pred_key]
    if isinstance(pred_str, list):
        pred_str = " ".join(pred_str)
    ans = item['answer']
    options_dict = extract_option_dict(item['messages'][0]['content'])
    if pred_str in "ABCD":
        tag = pred_str
        return tag == ans
    else:
        tag = pred_str.strip("$")
        try:
            ans = options_dict[ans]
        except:
            print(options_dict, ans, pred_str)
            return False
        new_item = item.copy()
        new_item.update({
            pred_key: tag,
        ...
        return is_correct(new_item, pred_key=pred_key, prec=prec)
